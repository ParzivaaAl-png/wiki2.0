      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed left-4 right-4 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:w-96 mt-2 bg-white dark:bg-neutral-950 border border-neutral-200/60 dark:border-neutral-850/80 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col"
          >
            <div className="flex border-b border-neutral-150 dark:border-neutral-900 bg-neutral-50/25 dark:bg-neutral-900/30 shrink-0 select-none">
              <button
                type="button"
                onClick={() => {
                  setActiveTab('news');
                ...
                className={`flex-1 py-2.5 text-center text-[10px] font-bold uppercase tracking-wider transition-all border-b-2 cursor-pointer ${
                  activeTab === 'news'
                    ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400'
                    : 'border-transparent text-neutral-500 hover:text-neutral-900 dark:hover:text-white'
                }`}
              >
                Новости {unreadNewsCount > 0 && `(${unreadNewsCount})`}
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-[220px]">
                  {searchQuery.trim().length >= 2 ? (
                    ...
                    newsList.length === 0 ? (
                      <div className="py-12 px-4 text-center text-neutral-400 select-none flex flex-col items-center justify-center gap-2 animate-fadeIn">
                      ...
                      <div className="divide-y divide-neutral-100 dark:divide-neutral-900 animate-fadeIn">
                        {newsList.map((news) => {
              ...
              <>
                ...
                {unreadNotificationsCount > 0 && (
                  <div className="px-3.5 py-2 border-b border-neutral-100 dark:border-neutral-900 flex justify-between items-center bg-neutral-50/30 dark:bg-neutral-950 shrink-0 select-none animate-fadeIn">
                    <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Системные оповещения</span>
                    <button
                      type="button"
                      onClick={handleMarkAllNotificationsRead}
                      className="text-[10px] text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 font-semibold cursor-pointer select-none"
                    >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-[220px]">
                  {notifications.length === 0 ? (
                    <div className="py-12 px-4 text-center text-neutral-400 select-none flex flex-col items-center justify-center gap-2 animate-fadeIn">
                      <Inbox className="w-8 h-8 text-neutral-300 dark:text-neutral-800" />
                      <p className="text-xs font-medium text-neutral-450 dark:text-neutral-650">Оповещений пока нет</p>
                    </div>
                  ) : (
                    <div className="divide-y divide-neutral-100 dark:divide-neutral-900 animate-fadeIn">
                      {notifications.map((notif) => {
                        ...
                        return (
                          <div
                            key={`notif-${notif.id}`}
                            className={`p-3.5 flex flex-col gap-1.5 relative transition-all ${
                              isUnread 
                                ? 'bg-indigo-500/[0.02] dark:bg-indigo-500/[0.01] border-l-2 border-indigo-500' 
                                : 'hover:bg-neutral-50/70 dark:hover:bg-neutral-900/30'
                            }`}
                          >
                            <div className="flex items-start justify-between gap-1.5">
                              <h4 className={`text-xs font-bold leading-tight ${isUnread ? 'text-neutral-950 dark:text-white' : 'text-neutral-700 dark:text-neutral-350'}`}>
                                {notif.title}
                              </h4>
                              <span className="text-[9px] text-neutral-400 font-medium shrink-0">
                            ...
                            <p className="text-[10px] text-neutral-500 dark:text-neutral-400 whitespace-pre-wrap leading-normal font-light">
                              {notif.message}
                            </p>
                          </div>
                        );
            ...
            {/* Footer */}
            <div className="px-4 py-2 border-t border-neutral-105 dark:border-neutral-900 text-center select-none bg-neutral-50/20 dark:bg-neutral-900/10 shrink-0">
