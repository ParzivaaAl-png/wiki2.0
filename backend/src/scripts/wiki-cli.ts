...
const envLocalPath = path.resolve(rootDir, '.env.codex.local');
...
export class CodexWikiCLI {
  ...
  async convertDocumentToWikiHtml(filePath: string): Promise<{ title: string; htmlContent: string; summary: string }> {
    ...
    if (ext === '.docx' || ext === '.doc') {
      ...
      const paragraphs: string[] = pdfData.text
        .split(/\n\s*\n/)
        .map((p: string) => p.trim())
        .filter(Boolean);
      ...
      if (content.trim().startsWith('<')) {
        ...
        rawHtml = paragraphs.map((p: string) => `<p>${this.escapeHtml(p.trim()).replace(/\n/g, '<br/>')}</p>`).join('\n');
      }
    }
  ...
  (async () => {
    try {
      if (command === 'publish') {
        ...
        if (!filePath) {
          console.log('Usage: npm run wiki:publish -- <path-to-file> [--sectionId=<id>] [--title="..."]');
          process.exit(1);
        }
